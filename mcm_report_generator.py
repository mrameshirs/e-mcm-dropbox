...
class PDFReportGenerator:
    ...
    def add_monthly_performance_summary(self):
        ...
        try:
            ...
            dars_submitted = self.vital_stats.get('num_dars', 11)
            revenue_involved = self.vital_stats.get('total_detected', 8018.32)
            revenue_recovered = self.vital_stats.get('total_recovered', 15.20)
            
            # Create metrics section using a table for better layout
            metrics_data = [
                ['✅ DARs Submitted', '💰 Revenue Involved', '💎 Revenue Recovered'],
                [f'{dars_submitted}', f'₹{revenue_involved:.2f} L', f'₹{revenue_recovered:.2f} L']
            ]
            ...
            if categories_summary:
                ...
                performance_data.append([
                    '📊 Total (All)',
                    str(total_dars),
                    f'₹{total_detected:,.2f} L',
                    f'₹{total_recovered:,.2f} L',
                    str(total_paras)
                ])
            else:
                ...
                performance_data = [
                    ...
                    ['Large', '5', '₹7,952.56 L', '₹0.00 L', '44'],
                    ['Medium', '5', '₹36.50 L', '₹15.20 L', '41'],
                    ['Small', '0', '₹0.00 L', '₹0.00 L', '0'],
                    ['📊 Total (All)', '10', '₹7,989.06 L', '₹15.20 L', '85']
                ]
