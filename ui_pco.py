...
def pco_dashboard(dbx):
    ...
    # ========================== VISUALIZATIONS TAB ==========================
        ...
        if df_unique_reports.empty:
            st.error("No unique reports data available")
            return
        ...
        try:
            ...
            # Debug output
            st.write("DEBUG - DAR Summary:")
            st.dataframe(dar_summary)
            
        except Exception as e:
            st.error(f"Error creating DAR summary: {e}")
            return
        ...
        try:
            ...
            # Debug output
            st.write("DEBUG - Para Summary:")
            st.dataframe(para_summary)
            
        except Exception as e:
            st.error(f"Error creating para summary: {e}")
            para_summary = pd.DataFrame(columns=['category', 'num_audit_paras'])
        ...
        try:
            ...
            # Reorder according to categories_order and add missing categories
            summary_df_ordered = []
            ...
            # Debug: Show the final summary dataframe
            st.write("DEBUG - Final Summary DataFrame:")
            st.dataframe(summary_df)
            
        except Exception as e:
            st.error(f"Error processing summary data: {e}")
            return
        ...
        try:
            ...
            for index, row in summary_df.iterrows():
                ...
                row_class = "total-row" if "Total" in str(row['category']) else ""
                html_rows += f"""
                <tr class="{row_class}">
                    <td class="text-data">{row['category']}</td>
            ...
            # Complete HTML table with embedded CSS
            html_table = f"""
            <style>
                .performance-table {{
                    border-collapse: collapse;
                ...
                .performance-table td.num-data {{
                    text-align: right;
                    font-family: 'Courier New', monospace;
                    font-weight: 500;
                }}
                .performance-table td.text-data {{
                    text-align: left;
                    font-weight: 500;
                }}
            ...
            st.error(f"Error creating HTML table: {e}")
