...
def pco_dashboard(dbx):
    ...
    if selected_tab == "Create MCM Period":
        ...
        st.markdown("#### Treemap Visualizations")
        if 'Detection in Lakhs' in df_unique.columns and 'category' in df_unique.columns:
            df_det_treemap = df_unique[df_unique['Detection in Lakhs'] > 0]
            if not df_det_treemap.empty:
                st.write("**Detection Amounts by Trade Name**")
                try:
                    fig_treemap = px.treemap(
                        ...
                        color='category',
                        color_discrete_map={
                            'Large': '#ff6b6b', 'Medium': '#4ecdc4', 
                            'Small': '#45b7d1', 'Unknown': '#96ceb4'
                        }
                    )
                    fig_treemap.update_layout(
                        title="Detection by Trade Name & Category",
                        margin=dict(t=50, l=25, r=25, b=25)
                    )
                    fig_treemap.update_traces(
                        textinfo="label+value",
                        hovertemplate="<b>%{label}</b><br>Amount: ₹%{value:.2f} L<extra></extra>"
                    )
                    st.plotly_chart(fig_treemap, use_container_width=True)
                except Exception as e:
                    st.error(f"Could not generate detection treemap: {e}")

        if 'Recovery in Lakhs' in df_unique.columns and 'category' in df_unique.columns:
            df_rec_treemap = df_unique[df_unique['Recovery in Lakhs'] > 0]
            if not df_rec_treemap.empty:
                st.write("**Recovery Amounts by Trade Name**")
                try:
                    fig_treemap_rec = px.treemap(
                        df_rec_treemap, 
                        path=[px.Constant("All"), 'category', 'trade_name'], 
                    ...
                    fig_treemap_rec.update_layout(
                        title="Recovery by Trade Name & Category",
                        margin=dict(t=50, l=25, r=25, b=25)
                    )
                    fig_treemap_rec.update_traces(
                        textinfo="label+value",
                        hovertemplate="<b>%{label}</b><br>Amount: ₹%{value:.2f} L<extra></extra>"
                    )
