...
# Using Dropbox utilities instead of Google utilities
from dropbox_utils import (
    read_from_spreadsheet,
    update_spreadsheet_from_df,
    upload_file
)
# Using the provided processing and validation logic
...
# Using the same config and models as the rest of the app
from config import (
    USER_CREDENTIALS,
...
# This list defines the final column order in the master data Excel file.
...
# This defines the column order and configuration for the st.data_editor widget.
...
def get_active_mcm_periods(dbx):
    ...
    # Create a unique key for each period, e.g., "July_2025"
...
def audit_group_dashboard(dbx):
    ...
    with st.sidebar:
        ...
        if st.button("Logout", key="ag_logout", use_container_width=True):
            # Clear all session state keys before logging out
...
def upload_dar_tab(dbx, active_periods, api_key):
    """Renders the 'Upload DAR' tab and handles its logic."""
    ...
    # Create a mapping for the selectbox display
    ...
    selected_period_str = st.selectbox(
        ...
        options=list(period_select_map_rev.keys()),
        key=f"ag_mcm_sel_uploader_{st.session_state.ag_uploader_key_suffix}"
    )

    if not selected_period_str:
        st.info("Please select an MCM period to begin.")
        return

    # --- State Management on Period Change ---
    ...
    # --- File Uploader and Data Extraction ---
    ...
    if uploaded_file and st.session_state.ag_current_uploaded_file_name != uploaded_file.name:
        # New file has been uploaded, reset state for this file
    ...
    if st.session_state.ag_current_uploaded_file_obj and st.button("Extract Data from PDF", use_container_width=True):
        with st.spinner(f"Processing '{st.session_state.ag_current_uploaded_file_name}'..."):
            ...
            if not upload_file(dbx, pdf_bytes, pdf_dropbox_path):
                st.error("Failed to upload PDF to Dropbox. Cannot proceed.")
                st.stop()
            
            st.session_state.ag_pdf_dropbox_path = pdf_dropbox_path
            st.success(f"DAR PDF uploaded successfully to Dropbox.")
            ...
            if preprocessed_text.startswith("Error"):
                ...
                parsed_data = ParsedDARReport() # Empty report
            else:
            ...
            # 3. Format extracted data for the editor
            ...
            if parsed_data.audit_paras:
                for para_obj in parsed_data.audit_paras:
                    ...
                    row = base_info.copy()
                    row.update(para_dict)
                    temp_list_for_df.append(row)
            elif base_info.get("trade_name"):
                row = base_info.copy()
                row.update({"audit_para_number": None, "audit_para_heading": "N/A - Header Info Only", "status_of_para": None})
                temp_list_for_df.append(row)
            else:
                row = base_info.copy()
                row.update({"audit_para_heading": "Manual Entry Required"})
                temp_list_for_df.append(row)
            ...
            st.success("Data extraction complete. Review and edit below.")
            st.rerun()

    # --- Data Editor and Submission Logic ---
    if not st.session_state.ag_editor_data.empty:
        st.markdown("<h4>Review and Edit Extracted Data:</h4>", unsafe_allow_html=True)
        
        # Configure the data editor columns
        col_conf = {
        ...
        # Use a stable key for the editor to preserve edits during the session
        ...
        edited_df = st.data_editor(
            ...
            hide_index=True,
            height=min(len(st.session_state.ag_editor_data) * 45 + 70, 450)
        )

        if st.button("Validate and Submit to MCM Sheet", use_container_width=True):
            df_to_submit = pd.DataFrame(edited_df).dropna(how='all').reset_index(drop=True)
            
            if df_to_submit.empty:
                st.error("Cannot submit, no data found in the editor.")
                return

            # Perform validation using the provided validation_utils.py logic
            ...
            with st.spinner("Submitting data to Dropbox..."):
                # Read-Modify-Write pattern for Dropbox
                master_df = read_from_spreadsheet(dbx, MCM_DATA_PATH)
                if master_df is None: # If read fails
                     st.error("Failed to read the master data file from Dropbox. Submission aborted.")
                     return

                # Prepare new data for concatenation
                ...
                if update_spreadsheet_from_df(dbx, final_df, MCM_DATA_PATH):
                    ...
                    st.error("Failed to save the updated data to Dropbox.")
...
def view_uploads_tab(dbx, all_periods):
    ...
    period_options_map = {f"{p.get('month_name')} {p.get('year')}": f"{p.get('month_name')} {p.get('year')}"
                          for _, p in all_periods.items()}
    ...
    with st.spinner("Loading your uploaded reports..."):
        ...
        if df_all_data.empty:
            st.info(f"No reports have been submitted for any period yet.")
            return

        # Ensure numeric type for correct filtering
        df_all_data['audit_group_number'] = pd.to_numeric(df_all_data['audit_group_number'], errors='coerce')

        my_uploads = df_all_data[
            (df_all_data['audit_group_number'] == st.session_state.audit_group_no) &
            (df_all_data['mcm_period'] == selected_period)
        ]
        ...
        display_cols = [
            "gstin", "trade_name", "audit_para_number", "audit_para_heading", "status_of_para",
...
def delete_entries_tab(dbx, all_periods):
    ...
    st.error("⚠️ **Warning:** This action is permanent and cannot be undone. It removes the entry from the master data file.")
    ...
    my_entries = master_df[
        (master_df['audit_group_number'] == st.session_state.audit_group_no) &
        (master_df['mcm_period'] == selected_period)
    ].copy()
...
# from dropbox_utils import read_from_spreadsheet, update_spreadsheet_from_df, upload_file
# from dar_processor import preprocess_pdf_text
# from gemini_utils import get_structured_data_with_gemini
# from validation_utils import validate_data_for_sheet, VALID_CATEGORIES, VALID_PARA_STATUSES
# from config import USER_CREDENTIALS, AUDIT_GROUP_NUMBERS, MCM_DATA_PATH, DAR_PDFS_PATH, MCM_PERIODS_INFO_PATH
# from models import ParsedDARReport
# from streamlit_option_menu import option_menu
...
#     active_periods = mcm_periods[mcm_periods['active'] == True] if not mcm_periods.empty else pd.DataFrame()
...
#         else:
#             period_options = {f"{row['month_name']} {row['year']}": row for index, row in active_periods.iterrows()}
#             selected_period_str = st.selectbox("Select Active MCM Period", options=list(period_options.keys()))

#             if selected_period_str:
#                 mcm_info = period_options[selected_period_str]
#                 uploaded_file = st.file_uploader("Choose DAR PDF", type="pdf")

#                 if uploaded_file:
#                     if st.button("Extract and Submit Data", use_container_width=True):
#                         with st.spinner("Processing PDF and submitting..."):
#                             pdf_bytes = uploaded_file.getvalue()
                            
#                             pdf_filename = f"{st.session_state.audit_group_no}_{uploaded_file.name}"
#                             pdf_path = f"{DAR_PDFS_PATH}/{pdf_filename}"
#                             if not upload_file(dbx, pdf_bytes, pdf_path):
#                                 st.error("Failed to upload PDF to Dropbox.")
#                                 st.stop()

#                             preprocessed_text = preprocess_pdf_text(BytesIO(pdf_bytes))
#                             parsed_data = get_structured_data_with_gemini(YOUR_GEMINI_API_KEY, preprocessed_text)
...
#                             base_info = {
#                                 "audit_group_number": st.session_state.audit_group_no,
#                                 "audit_circle_number": math.ceil(st.session_state.audit_group_no / 3.0),
#                                 "gstin": header_dict.get("gstin"),
#                                 "trade_name": header_dict.get("trade_name"),
#                                 "category": header_dict.get("category"),
#                                 "total_amount_detected_overall_rs": header_dict.get("total_amount_detected_overall_rs"),
#                                 "total_amount_recovered_overall_rs": header_dict.get("total_amount_recovered_overall_rs"),
#                                 "dar_pdf_path": pdf_path,
#                                 "mcm_period": selected_period_str,
...
#                             else:
#                                 rows_to_add.append(base_info)
                            
#                             new_data_df = pd.DataFrame(rows_to_add)
                            
#                             mcm_data = read_from_spreadsheet(dbx, MCM_DATA_PATH)
#                             updated_mcm_data = pd.concat([mcm_data, new_data_df], ignore_index=True)
#                             if update_spreadsheet_from_df(dbx, updated_mcm_data, MCM_DATA_PATH):
#                                 st.success("Data submitted successfully!")
#                                 st.balloons()
#                             else:
...
#             my_uploads = mcm_data[mcm_data['audit_group_number'] == st.session_state.audit_group_no]
#             if not my_uploads.empty:
#                 st.dataframe(my_uploads, use_container_width=True)
#             else:
#                 st.info("You have not uploaded any DARs yet.")
#         else:
#             st.info("No DARs have been uploaded yet.")

#     elif selected_tab == "Delete My DAR Entries":
#         st.markdown("<h3>Delete My Uploaded DAR Entries</h3>", unsafe_allow_html=True)
#         st.info("⚠️ This action is irreversible. Deletion removes entries from the central data file.")
        
#         mcm_data = read_from_spreadsheet(dbx, MCM_DATA_PATH)
#         if not mcm_data.empty:
#             my_uploads = mcm_data[mcm_data['audit_group_number'] == st.session_state.audit_group_no].copy()
#             if not my_uploads.empty:
#                 my_uploads['display_str'] = my_uploads.apply(lambda row: f"{row['trade_name']} - {row['upload_timestamp']}", axis=1)
                
#                 options = ["--Select an entry to delete--"] + my_uploads['display_str'].tolist()
#                 selected_entry_str = st.selectbox("Select Entry:", options)

#                 if selected_entry_str != "--Select an entry to delete--":
#                     selected_row = my_uploads[my_uploads['display_str'] == selected_entry_str].iloc[0]
#                     st.warning(f"Confirm Deletion: **{selected_row['trade_name']}** uploaded on **{selected_row['upload_timestamp']}**")

#                     with st.form(key="delete_form"):
#                         pwd = st.text_input("Password:", type="password")
#                         if st.form_submit_button("Yes, Delete This Entry"):
#                             if pwd == USER_CREDENTIALS.get(st.session_state.username):
#                                 index_to_delete = selected_row.name
#                                 updated_mcm_data = mcm_data.drop(index_to_delete)
#                                 if update_spreadsheet_from_df(dbx, updated_mcm_data, MCM_DATA_PATH):
#                                     st.success("Entry deleted successfully.")
#                                     time.sleep(1)
#                                     st.rerun()
