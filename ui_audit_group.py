# Fixed version of the upload logic in upload_dar_tab function
...
def upload_dar_tab(dbx, active_periods, api_key):
    ...
    # FIX: Don't reset states when file changes, just update the file references
    if uploaded_file and st.session_state.ag_current_uploaded_file_name != uploaded_file.name:
        ...
        # REMOVED: reset_ag_states() - this was clearing the uploaded file
        # Only clear the editor data and related states, not the file itself
        ...
        # Clear widget states without reassigning
        for key in ['ag_taxpayer_classification', 'ag_no_risk_flags', 'new_risk_flag_select']:
            if key in st.session_state:
                del st.session_state[key]
        st.rerun()

    # FIX: Show Extract button when file is uploaded
    if st.session_state.ag_current_uploaded_file_obj:
...
# Alternative simpler fix - just update the reset_ag_states function:

def reset_ag_states():
    """Resets all session state variables for the audit group upload tab."""
    # DON'T clear the uploaded file references
    # st.session_state.ag_current_uploaded_file_obj = None  # ← REMOVE THIS LINE
    # st.session_state.ag_current_uploaded_file_name = None  # ← REMOVE THIS LINE
