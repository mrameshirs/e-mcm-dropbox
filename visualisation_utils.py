...
def get_visualization_data(dbx, selected_period):
    ...
    try:
            ...
            for code in all_classification_codes:
                ...
                if not df_agg.empty:
                    ...
                    fig_detailed_det.update_layout(
                        ...
                        xaxis=dict(
                            tickangle=-30,
                            tickfont=dict(size=9, family="serif", color='#5A4A4A'),
                            showgrid=False
                        ),
                        margin=dict(l=60, r=20, t=20, b=120),
                        height=380
        ...
        if not df_viz_data.empty:
            ...
            # df_mcm_data['total_amount_detected_overall_rs'] = pd.to_numeric(df_mcm_data['total_amount_detected_overall_rs'], errors='coerce').fillna(0)
            # df_mcm_data['total_amount_recovered_overall_rs'] = pd.to_numeric(df_mcm_data['total_amount_recovered_overall_rs'], errors='coerce').fillna(0)
